...
public:
    ...
    vector<int> nextValuablePositions(vector<IntPair>& sortedPairs) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            cout << "Stack after pushing (" << sortedPairs[i].first << ", " << sortedPairs[i].second << "): ";
            printStack(stk);
        }

        // Print the resulting 'res' vector
        cout << "Resulting 'res' vector: ";
        printVector(res);
    ...
    void printStack(stack<IntPair> stk) {
        vector<IntPair> temp;
        while (!stk.empty()) {
