...
public:
    ...
    int maxDotProduct(vector<int>& nums1, vector<int>& nums2) {
        cache.resize(nums1.size()+1,vector<int>(nums2.size()+1,-1));
    ...
    int solve(vector<int>& nums1,vector<int>& nums2,int index1,int index2) {
        ...
        int ans = INT_MIN;
        ans = max(ans,solve(nums1,nums2,index1+1,index2));
        ans = max(ans,solve(nums1,nums2,index1,index2+1));
        ans = max(ans,nums1[index1]*nums2[index2]+max(0,solve(nums1,nums2,index1+1,index2+1)));
