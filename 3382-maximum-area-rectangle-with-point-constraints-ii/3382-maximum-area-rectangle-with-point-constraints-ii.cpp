...
public:
    long long maxRectangleArea(vector<int>& x, vector<int>& y) {
        ...
        SegTree* root = new SegTree(minX, maxX);
        int n = x.size();
        for (int i = 0; i < n; i++) {
            mp[y[i]].push_back(x[i]);
        }

        long long ans = -1; // Initialize with -1 for no valid rectangle
        for (auto [y, xs] : mp) {
            ...
            for (int i = 0; i < xs.size() - 1; i++) {
                ...
                if (y1 == y2 && y1 != y && y1 != INT_MIN) {
                    ...
                    if (ymid < y1) {
                        ans = max(ans, static_cast<long long>(xs[i + 1] - xs[i]) * static_cast<long long>(abs(y1 - y)));
                    }
                }
            }
